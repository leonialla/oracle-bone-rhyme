  ...
  <Teleport to="body">
    ...
    <Transition name="dialog-visible" @transitionend="trapFocusDialog">
      <div
        ...
        fixed inset-0 of-y-auto overscroll-none scrollbar-hide
      >
        ...
        <!-- Mask layer: blur -->
        <div class="dialog-mask" absolute inset-0 z-0 touch-none bg-transparent opacity-100 backdrop-blur-sm backdrop-filter />
        <!-- Mask layer: dimming -->
        <div class="dialog-mask" absolute inset-0 z-0 touch-none bg-black opacity-48 h="[calc(100%+0.5px)]" @click="clickMask" />
        <!-- Dialog container -->
        <div class="p-safe-area" pointer-events-none absolute inset-0 z-1 flex opacity-100>
          <div flex flex-1 items-center justify-center p-4>
            <!-- We use `class` here to make v-bind being able to be override them -->
            <div
              ref="elDialogMain"
              class="dialog-main pointer-events-auto isolate max-h-full w-full touch-pan-x touch-pan-y of-y-auto overscroll-contain bg-white rounded text-gray-8"
